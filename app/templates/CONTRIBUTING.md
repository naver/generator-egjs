# How to contribute to egjs-<%= componentname %>
egjs-<%= componentname %> is opened to everyone and we're welcoming for any kind of contribution.
We believe that our project can grow with your interests helping others' necessities.

## Style Guide

egjs-<%= componentname %> has several style guidelines to follow.
...
egjs-<%= componentname %> use commit logs in many different purposes (like creating CHANGELOG, ease history searching, etc.).
...
1. Fork `egjs-<%= componentname %>` on your repository
...
9. Create a new PR from your branch to egjs-<%= componentname %>.
10. Wait for reviews.
...
## License
By contributing to egjs-<%= componentname %>, you're agreeing that your contributions will be licensed under its [MIT](https://opensource.org/licenses/MIT) license.